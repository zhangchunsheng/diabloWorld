...
{   
    ...
    player_dic->retain();
    //CCSpriteFrameCache::sharedSpriteFrameCache()->addSpriteFramesWithFile("begin1.plist");
    //chatBounder=CCLabelTTF::create("", "Marker Felt", 30, CCSizeMake(560, 400), kCCTextAlignmentLeft);
    
    
    AnimatePacker :: getInstance()->loadAnimations("qishouTX.xml");
    //AnimatePacker::getInstance()->loadAnimations("tx.xml");
    ...
    bool flaag=false;
     ...
     //audionum=CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect("bg2.mp3");
...
void Fire::MeasureEneDead(CCObject * eneTag){
    ...
    if(changeHp+currHp<=0){
        ...
        //        CCActionInterval * de=CCDelayTime::create(2);
        //        CCCallFunc * func=CCCallFunc::create(this, callfunc_selector(Fire::checkOut));
        //        CCFiniteTimeAction * final=CCSequence::create(de,func,NULL);
...
void Fire::changeBlood(){
    ...
    CCSprite * nl=(CCSprite *)enemy->getChildByTag(24);
        ...
        for (int i=0; i<startValue.size(); i++) {
            ...
            if(direction==1){
                ...
                if(pos>=1&&pos<=3){
                    ...
                    switch (pinzhi) {
                        case 1:
                            player1->initWithFile("common_qs_bkg_4.png");
                            break;
                        case 2:
                            player1->initWithFile("common_qs_bkg_4.png");
                            break;
                    ...
                    player1->setPosition(ccp(size.width/2+(pos-2)*150, size.height/2-50));
                ...
                if(pos>6&&pos<=9){
                    ...
                    player111->setScale(0.38);
                    player1->setTag(direction*10+pos);
                    player1->setPosition(ccp(size.width/2+(pos-8)*150, size.height/2-320));
                    player111->setAnchorPoint(ccp(0, 0));
                    player1->addChild(player111);
                    char  pinz[40]="";
                    this->addChild(player1);
                ...
                if(pos>=1&&pos<=3){
                    ...
                    ene1->setPosition(ccp(size.width/2+(pos-2)*150, size.height/2+90));
                ...
                if (pos>3&&pos<=6) {
                    ...
                    ene1->setPosition(ccp(size.width/2+(pos-5)*150, size.height/2+220));
                    ene111->setAnchorPoint(ccp(0, 0));
                    ene1->addChild(ene111);
                    this->addChild(ene1);
                    char  pinz[40]="";
                ...
                if(pos>6&&pos<=9){
                    ...
                    ene1->setPosition(ccp(size.width/2+(pos-8)*150, size.height/2+350));
                    ene111->setAnchorPoint(ccp(0, 0));
                    ene1->addChild(ene111);
                    this->addChild(ene1);
                    char  pinz[40]="";
