...
//            item->autorelease();
            item->nickname = root[i]["name"].asString();
            item->desc = root[i]["desc"].asString();
            item->bid = root[i]["id"].asInt();
            item->exp = root[i]["exp"].asInt();
            item->coin = root[i]["coin"].asInt();
...
void CData::setyinyue(char *yinyue)
{
